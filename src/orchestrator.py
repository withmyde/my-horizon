...
class HorizonOrchestrator:
    ...
    async def fetch_all_sources(self, since: datetime) -> List[ContentItem]:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if self.config.sources.arxiv and self.config.sources.arxiv.enabled:
                sent_papers_path = Path("data/sent_arxiv.json")
                arxiv_scraper = ArxivScraper(self.config.sources.arxiv, client, sent_papers_path)
    ...
    def _sub_source_label(item: ContentItem) -> str:
        ...
        if meta.get("arxiv_category"):
            return f"arXiv:{meta['arxiv_category']}"
